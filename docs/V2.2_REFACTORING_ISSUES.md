...
1. **File preview generation** (LLM column hallucination) - ✅ FIXED
2. **Database observability** (8/10 tables empty) - ❌ NOT FIXED
3. **Column metadata persistence** (enrichment lost) - ✅ FIXED
4. **Canonicalization integration** (period-embedded codes) - ✅ FIXED
5. **Debug output pollution** (console clutter) - ❌ NOT FIXED
...
**Status (2026-01-13):** 3 out of 5 issues fixed. Database observability and debug output cleanup remain.
...
def generate_manifest(url, output_path, skip_preview=False):
    ...
    # Write YAML
    # ❌ PREVIEW GENERATION CODE DELETED
    # ❌ skip_preview parameter exists but does nothing
```
...
def process_period(pub_code, pub_config, period, url, event_store):
    ...
    for source in manifest['sources']:
        ...
        if result.success:
            ...
            # ❌ repository.record_manifest_file() NOT CALLED
```
...
|-------|---------|--------|----------|
| `tbl_enrichment_runs` | Track LLM enrichment calls | ❌ 0 rows | Schema exists, never populated |
| `tbl_enrichment_api_calls` | LLM API metrics (tokens, cost) | ❌ 0 rows | Schema exists, never populated |
| `tbl_manifest_files` | Per-file load tracking | ❌ 0 rows | Schema exists, never populated |
| `tbl_pipeline_log` | Pipeline event log | ❌ 0 rows | Schema exists, never populated |
| `tbl_source_mappings` | LLM code → canonical mapping | ❌ 0 rows | Phase 1 feature, never used |
| `tbl_canonical_sources` | Canonical source registry | ❌ 0 rows | Phase 1 feature, never used |
| `tbl_drift_events` | Schema change tracking | ❌ 0 rows | Phase 1 feature, never used |
| `tbl_column_metadata` | Column descriptions | ❌ 0 rows | Fixed 2026-01-13 (commit b5785da) |
...
class EventStore:
    def emit(self, event: Event):
        ...
        self._emit_console(event)    # ✅ Print to stdout
        self._emit_file_log(event)   # ✅ Write to logs/backfill_*.log
        self._emit_jsonl(event)      # ✅ Write to logs/events/*.jsonl
        # self._emit_database(event) # ❌ DOES NOT EXIST
```
...
def record_manifest_file(...):           # ❌ Never called
def check_manifest_file_status(...):     # ❌ Never called
def get_manifest_summary(...):          # ❌ Never called
def get_manifest_errors(...):           # ❌ Never called
def get_manifest_files(...):            # ❌ Never called
```
...
❌ **NOT FIXED** - Database observability completely broken.
...
```
1. Generate manifest → CSV/Excel files (no columns yet)
2. Add preview → Download files, extract columns
3. LLM enrichment → Add descriptions, semantic names
4. Load data → Store to PostgreSQL
5. Store metadata → Persist enriched columns to tbl_column_metadata
6. Export Parquet → Include enriched descriptions in .md file
```

### What Actually Happened (v2.2)

```
1. Generate manifest → ✅ Works
2. Add preview → ❌ BROKEN (Issue #1)
3. LLM enrichment → ❌ Hallucinates columns (no preview data)
4. Load data → ✅ Works (uses actual columns from Extractor)
5. Store metadata → ❌ NOT IMPLEMENTED
6. Export Parquet → ❌ Generates .md without descriptions
```
...
**Status:** ✅ FIXED (commit b5785da) - But only tested with fresh data, not verified end-to-end
...
This enables cross-period consolidation (May + November → same table).
...
```
Period 1 (May):  Enrich without reference → adhd_may25_data → Canonicalize → adhd_data
Period 2 (Nov):  Enrich WITH reference (May) → Match by URL → Reuse adhd_data code
```
...
⚠️ **PARTIAL** - Canonicalization works for first period, but reference-based matching for subsequent periods is untested.

---

## Issue #5: Debug Output Pollution

### What Happens

Console output during backfill is polluted with debug statements that should only go to log files:

```
INFO:   [OK] ddl completed
INFO:   [OK] load completed
⚠️  Low row count: 13 rows loaded to staging.tbl_summary_table_5b (expected >100)
INFO:   Step: load
DEBUG CELL TYPES: date_val → types: {'d'}
DEBUG TYPE INFERENCE: date_val
  Raw values (first 10): ['2025-06-01 00:00:00', '2025-05-01 00:00:00', ...]
  Clean values (first 10): ['2025-06-01 00:00:00', '2025-05-01 00:00:00', ...]
  Total raw: 13, Total clean: 13
DEBUG CELL TYPES: age_0_to_4 → types: {'n'}
DEBUG TYPE INFERENCE: age_0_to_4
  Raw values (first 10): ['165', '185', '185', '285', '225', ...]
```

### Root Cause

**File:** `src/datawarp/core/extractor.py`

Debug print statements were left in production code:
...
def _infer_column_types(self, df: pd.DataFrame) -> dict:
    ...
    for col_name in df.columns:
        # Get cell types
        cell_types = self._classify_cells(df[col_name])

        # ❌ DEBUG OUTPUT TO CONSOLE
        print(f"DEBUG CELL TYPES: {col_name} → types: {cell_types}")

        # Infer type
        inferred = self._infer_from_cell_types(cell_types)

        # ❌ MORE DEBUG OUTPUT
        print(f"DEBUG TYPE INFERENCE: {col_name}")
        print(f"  Raw values (first 10): {df[col_name].head(10).tolist()}")
```

### What Should Happen

Debug output should use Python's `logging` module with level control:
...
def _infer_column_types(self, df: pd.DataFrame) -> dict:
    ...
    for col_name in df.columns:
        ...
        logger.debug(f"Cell types for {col_name}: {cell_types}")

        inferred = self._infer_from_cell_types(cell_types)

        logger.debug(f"Type inference for {col_name}: {inferred}")
        logger.debug(f"Sample values: {df[col_name].head(10).tolist()}")
```

**Result:** Console shows only INFO+ messages, DEBUG goes to log files only.

### Impact

- Console output is cluttered and hard to read
- Production logs are polluted with debug info
- Users can't see important summary messages
- Looks unprofessional

### Status

❌ **NOT FIXED** - Requires cleanup of print statements in extractor.py

### Fix Required

**Search and replace in `src/datawarp/core/extractor.py`:**
1. Replace `print(f"DEBUG ...)` with `logger.debug(...)`
2. Remove or reduce sample value output
3. Add logging configuration in `__init__()`
4. Ensure logging respects level (DEBUG vs INFO)
...
- ✅ EventStore console output (clean, structured)
- ✅ File-based logging (.log, .jsonl)
- ✅ Code organization (pipeline library)
...
def main(url, output_path, enable_preview=True):  # ← Default: True
    resources = scrape_resources(url)
    processed = process_resources(resources, enable_preview=True)  # ← Preview ON
...
def generate_manifest(url, output_path, skip_preview=False):  # ← Never used
    resources = scrape_resources(url)
    processed = process_resources(resources)  # ← No preview generation
...
for file in manifest['files']:
    ...
    repository.record_manifest_file(manifest, file, result)  # ← DB tracking
...
for file in manifest['files']:
    result = load_file(file)
    event_store.emit(create_event(...))  # ← File tracking only
    # ❌ No repository.record_manifest_file()
```
...
The v2.2 refactoring improved architecture but broke critical features through incomplete implementation. Five major issues discovered:

1. ✅ **Preview generation** - FIXED (2026-01-13, commit de77cf9)
2. ❌ **Database observability** - NOT FIXED (8/10 tables empty, EventStore has no database sink)
3. ✅ **Column metadata** - FIXED (2026-01-13, commit b5785da)
4. ✅ **Canonicalization** - FIXED (2026-01-13, commits cb9819d, 1115de6)
5. ❌ **Debug output** - NOT FIXED (print statements in extractor.py)

**Current status:** System works for loading data with accurate metadata but lacks database observability. Console output polluted with debug messages.

**Recommendation for Opus:**
- **Priority 1:** Implement Option A (complete EventStore database sink) or Option C (hybrid approach)
- **Priority 2:** Clean up print statements in extractor.py, replace with proper logging
- **Priority 3:** Test reference-based enrichment for subsequent periods
